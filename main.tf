...
resource "aws_launch_configuration" "retail" {
  ...
  user_data = <<-EOF
              ...
              docker pull tsivabe/retail_django:retailweb1
              docker run -p 8000:8000 --name retail1 tsivabe/retail_django:retailweb1 gunicorn RetailCom.wsgi:application --bind 0.0.0.0:8000
              EOF
